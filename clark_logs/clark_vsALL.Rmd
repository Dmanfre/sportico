...
subset_clark_logs <- clark_logs_cleaned %>% 
  group_by(opponent) %>% 
...
clark_logs_and_games <- inner_join(subset_clark_logs, games_per_team, by = "opponent")

# per game anaysis

clark_logs_and_games <- clark_logs_and_games %>% 
  group_by(opponent) %>% 
...
write_csv(clark_logs_and_games, "data/cclark_logs_and_games.csv")
```
